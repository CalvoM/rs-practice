...
pub fn question_1() {
    ...
    let mut counter_values: HashMap<u32, u32> = HashMap::from([(1, 3), (2, 1), (3, 2)]);
...
pub fn question_2() {
    ...
    let mut win_values: HashMap<u32, u32> = HashMap::from([(1, 3), (2, 1), (3, 2)]);
    let mut loss_values: HashMap<u32, u32> = HashMap::from([(1, 2), (2, 3), (3, 1)]);
    let contents = fs::read_to_string("./src/day_two/input.txt").expect("Reading not okay");
    let results = contents.trim().split("\n");
    let mut total_score: u64 = 0;
    for result in results {
